...
def get_args():
    p = argparse.ArgumentParser(description="MIMIC-IV sliding window preprocessing")
    p.add_argument("--max_los_hours", type=int, default=168,
                   help=""
    p.add_argument("--slide_step",    type=int, default=6,
                   help="")
    p.add_argument("--past_hours",    type=int, default=36,
                   help="")
    p.add_argument("--future_hours",  type=int, default=12,
                   help="")
    p.add_argument("--out_dir",       type=str, default="./processed_36_12_sliding",
                   help="")
...
def build_cohort(args) -> pd.DataFrame:
    ...
    for sp in ["train","val","test"]:
        ...
        print(f"  {sp}: {len(sub):,} stays | mort={sub['died_in_hospital'].mean():.3f}")
...
def build_raw_grids(icu: pd.DataFrame, total_hours: int) -> dict:
    print("\n[3] Processing CHARTEVENTS → raw grids (total_hours=%d)..." % total_hours)
    stay_ids   = set(icu["stay_id"])
    icu_time   = icu[["stay_id","intime"]].set_index("stay_id")
...
def build_hourly_arrays(
    ...
    """
      label_counts : dict
    """
    total_hours = grid_hours if grid_hours is not None else (past_hours + future_hours)
    print(f"\n[5-6] Building hourly arrays "
          f"(grid={total_hours}h, past={past_hours}h / future={future_hours}h / total={total_hours}h)...")
    ...
    for sid in tqdm(stay_ids, desc="  assembling"):
        ...
        future_raw  = grid[past_hours : past_hours + future_hours, :]
...
def assemble_dataset(
    ...
    """
      ...
      LOS=72h, total=48h, step=6h

      historical_ts_numeric : grid[start:start+past]        (36, 61)
      forecasting_targets   : grid_fc[start:start+total]    (48,  3)  HR/RR/SpO2
    """
    print(f"\n[9] Assembling sliding window dataset "
          f"(past={past_hours}h, future={future_hours}h, step={slide_step}h)...")
    stay_to_label = dict(zip(icu["stay_id"].astype(int), icu["died_in_hospital"]))
    stay_to_split = dict(zip(icu["stay_id"].astype(int), icu["split"]))
    stay_to_los   = dict(zip(icu["stay_id"].astype(int),
                             icu["los_hours"].clip(upper=9999)))

    total_hours = past_hours + future_hours

    OBS_THRESH = past_hours * 0.3
    ...
    for sid in tqdm(normed_grids.keys(), desc="  assembling"):
        ...
        if sid not in masks_vital:                               skipped+=1; continue
        ...
        fc_full = grid[:, FORECAST_INDICES]
        obs_fc  = cnt[:, FORECAST_INDICES]

        actual_los = min(int(stay_to_los.get(sid, total_hours)), grid.shape[0])

        win_count = 0
        for start in range(0, actual_los - total_hours + 1, slide_step):
            end = start + total_hours

            obs_past = obs_fc[start : start + past_hours, :]
            if obs_past[:, 0].sum() < OBS_THRESH:
                continue
            if obs_past[:, 1].sum() < OBS_THRESH:
                continue
            if obs_past[:, 2].sum() < OBS_THRESH:
                continue

            fc_mask_w = (obs_fc[start + past_hours : end] > 0).astype(np.int32)
            ...
            dataset.append({
                "stay_id"              : sid,
                "window_start"         : start,
                "historical_ts_numeric": grid[start : start+past_hours].astype(np.float32),
                "obs_count"            : cnt_clip[start : start+past_hours],
                "hours_since"          : hs_norm[start : start+past_hours],
                "forecasting_targets"  : fc_full[start : end].astype(np.float32),
                "forecast_masks"       : fc_mask_w,
        ...
        if win_count == 0:
            obs_past_0 = obs_fc[:past_hours, :]
            if (obs_past_0[:, 0].sum() < OBS_THRESH or
                obs_past_0[:, 1].sum() < OBS_THRESH or
                obs_past_0[:, 2].sum() < OBS_THRESH):
                skipped += 1
                continue

            fc_mask_w = forecast_masks.get(
                sid, np.zeros((future_hours, N_FORECAST), dtype=np.int32))
            if fc_mask_w.sum() == 0:
                skipped += 1
                continue
            ...
            dataset.append({
                "stay_id"              : sid,
                "window_start"         : 0,
                "historical_ts_numeric": grid[:past_hours].astype(np.float32),
                "obs_count"            : cnt_clip[:past_hours],
                "hours_since"          : hs_norm[:past_hours],
                "forecasting_targets"  : fc_full[:total_hours].astype(np.float32),
                "forecast_masks"       : fc_mask_w,
                "static_numeric"       : stat["static_numeric"],
                "static_categoric"     : stat["static_categoric"],
                "label"                : int(stay_to_label[sid]),
        ...
        if win_count > 0:
            n_wins_list.append(win_count)

    avg_wins = float(np.mean(n_wins_list)) if n_wins_list else 0.0
    print(f"  → {len(dataset):,} windows from {len(n_wins_list):,} stays "
          f"(avg {avg_wins:.1f} wins/stay, skipped: {skipped:,})")
    return dataset
...
def main():
    ...
    TOTAL_HOURS  = PAST_HOURS + FUTURE_HOURS
    MAX_LOS_H = args.max_los_hours
    GRID_HOURS = MAX_LOS_H
    ...
    static_map = build_static_features(icu, GRID_HOURS)

    raw = build_raw_grids(icu, GRID_HOURS)

    vaso = build_vasopressor_grids(icu, GRID_HOURS)
