...
class CrossformerQuantile(nn.Module):
    """
    Crossformer backbone + Quantile head + Deterioration head.


      backbone(x) → (B, T_out, N_VARS)
      quantile_proj: (N_VARS → N_VARS × n_q) per timestep
      → reshape → (B, n_q, T_out, N_VARS)

    """
    def __init__(self, backbone: Crossformer, n_q: int, n_vars: int, future_hours: int):
        super().__init__()
...
def quantile_loss(pred, target, mask, quantiles=QUANTILES):
    """pred:(B,n_q,T,N)  target/mask:(B,T,N)"""
...
def get_args():
    ...
    p.add_argument("--seg_len",  default=6,   type=int,
        help="")
    p.add_argument("--win_size", default=2,   type=int,
        help="")
    p.add_argument("--factor",   default=10,  type=int,
        help="")
...
def main():
    ...
    print(f"Crossformer (ours): seg_len={args.seg_len} win={args.win_size} "
          f"d={args.d_model} [Quantile 5 paths]")

    backbone = Crossformer(
        data_dim  = N_VARS,
        in_len    = args.past_hours,
        out_len   = args.future_hours,
    ...
    if args.phase in ("train", "all"):
        ...
        print(f"\n{'='*55}\nTraining [Crossformer ours: Quantile, 5 paths]\n{'='*55}")
